...
def main():
    ...
    parser.add_argument("--config_file", help="json configuration file containint params to initialize the jetson", type=str)
    ...
    try:
        ...
        log.warning(LOGGER_OBJECT_DETECTOR_STARTUP, msg='Launching object detector now.')
        signal.signal(signal.SIGTERM, sigterm_handler)
        objectDetector = ObjectDetector(robotJetsonConfiguration)
        log.warning(LOGGER_OBJECT_DETECTOR_STARTUP, msg='Launching object detector load_model...')
        objectDetector.load_model()
        log.warning(LOGGER_OBJECT_DETECTOR_STARTUP, msg='Launching object detector run inference loop...')
        objectDetector.run()

    except SystemExit:
        log.info(LOGGER_OBJECT_DETECTOR_STARTUP, 'Caught SystemExit...')
    except:
        log.critical(LOGGER_OBJECT_DETECTOR_STARTUP, 'Crash in startup : {}'.format(traceback.print_exc()))
    finally:
        graceful_shutdown()
        logging.shutdown()
...
def sigterm_handler(sig, frame):
    log.info(LOGGER_OBJECT_DETECTOR_STARTUP, 'SIGTERM caught. Docker Container being terminated.')
    graceful_shutdown()
...
def graceful_shutdown():
    try:
        log.info(LOGGER_OBJECT_DETECTOR_STARTUP, 'Initiating Graceful Shutdown')
        global bot
        if 'bot' in globals():
        ...
        log.critical(LOGGER_OBJECT_DETECTOR_STARTUP, 'trace: {}'.format(traceback.print_exc()))
